...
export const SimilarCardsModal: React.FC<SimilarCardsModalProps> = ({
  ...
  const currentInspectIndex = useMemo(() => {
    if (!inspectCardMatch || !presentedMatches.length) return -1;
    return presentedMatches.findIndex(
      (m) =>
        m.card.id === inspectCardMatch.card.id ||
        (m.card.name === inspectCardMatch.card.name && m.card.set === inspectCardMatch.card.set)
    );
  }, [inspectCardMatch, presentedMatches]);

  const canNavigatePrevComp = currentInspectIndex > 0;
  const canNavigateNextComp = currentInspectIndex >= 0 && currentInspectIndex < presentedMatches.length - 1;

  const handlePrevComp = useCallback(() => {
    if (currentInspectIndex > 0) {
      setInspectCardMatch(presentedMatches[currentInspectIndex - 1]);
    }
  }, [currentInspectIndex, presentedMatches]);

  const handleNextComp = useCallback(() => {
    if (currentInspectIndex >= 0 && currentInspectIndex < presentedMatches.length - 1) {
      setInspectCardMatch(presentedMatches[currentInspectIndex + 1]);
    }
  }, [currentInspectIndex, presentedMatches]);
  ...
  // Keyboard navigation (Esc to close, ArrowLeft / ArrowRight to step through cards or comp precedents)
  useEffect(() => {
    if (!isOpen) return;
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      // Do not navigate cards if typing in an input/textarea
      const activeTag = (document.activeElement?.tagName || '').toLowerCase();
      if (activeTag === 'input' || activeTag === 'textarea' || swapSearchSlotIndex !== null) return;

      // When inspecting a comp card head-to-head, navigate between comp precedents
      if (inspectCardMatch) {
        if ((e.key === 'ArrowLeft' || e.key === '[') && canNavigatePrevComp) {
          e.preventDefault();
          handlePrevComp();
        } else if ((e.key === 'ArrowRight' || e.key === ']') && canNavigateNextComp) {
          e.preventDefault();
          handleNextComp();
        }
        return;
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-0 sm:p-5 bg-black/80 backdrop-blur-md animate-in fade-in duration-200 overflow-y-auto">
      <div 
        onClick={(e) => e.stopPropagation()}
        className="w-full h-full sm:w-[96vw] sm:max-w-[1600px] sm:max-h-[94vh] my-auto flex flex-col bg-white dark:bg-[#090e24] border-0 sm:border border-slate-200 dark:border-slate-800 rounded-none sm:rounded-3xl shadow-2xl overflow-hidden"
      >
        {/* Modal Header */}
        ...
        {/* Modal Body */}
        <div className="flex-1 overflow-y-auto p-3 sm:p-4 space-y-3 custom-scrollbar pb-safe">
          {!isReady ? (
      ...
      {inspectCardMatch && targetCard && (
        ...
        >
          ...
          >
                ...
                <div className="min-w-0">
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    {currentInspectIndex >= 0 && (
                      <span className="px-2.5 py-0.5 rounded-full text-xs font-bold font-mono bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 border border-slate-300 dark:border-slate-700 shrink-0">
                        Slot {currentInspectIndex + 1} of {presentedMatches.length}
                      </span>
                    )}
                  </div>
              ...
              <div className="space-y-4 pt-6 lg:pt-0 lg:pl-6">
                <div className="flex items-center justify-between pb-2.5 border-b border-slate-200 dark:border-slate-800">
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    {currentInspectIndex >= 0 && (
                      <span className="px-2 py-0.5 rounded-md bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 text-[11px] font-bold font-mono border border-slate-200 dark:border-slate-700">
                        Slot {currentInspectIndex + 1} of {presentedMatches.length}
                      </span>
                    )}
                  </div>
            ...
            {/* Modal Footer */}
            <div className="px-4 sm:px-6 py-2.5 border-t border-slate-200 dark:border-slate-800 text-xs text-slate-400 dark:text-slate-500 bg-slate-50/80 dark:bg-[#050818]/90 font-mono text-[11px] flex flex-wrap items-center justify-between gap-3">
              <div className="flex items-center gap-2 truncate">
                <span className="truncate">Head-to-head: {targetCard.name} vs. {inspectCardMatch.card.name}</span>
                <span className="hidden md:inline text-slate-400 dark:text-slate-600">•</span>
                <span className="hidden md:inline text-slate-400 dark:text-slate-500">Esc to close</span>
              </div>

              {/* Comp Precedent Navigation Controls (Bottom Right) */}
              <div className="flex items-center gap-2 ml-auto">
                <button
                  type="button"
                  onClick={handlePrevComp}
                  disabled={!canNavigatePrevComp}
                  title="Previous comparable precedent ([ or ←)"
                  className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl border text-xs font-sans font-semibold transition-all cursor-pointer disabled:opacity-40 disabled:cursor-not-allowed border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800/90 text-slate-700 dark:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-700 active:scale-95 shadow-2xs"
                >
                  <ChevronLeft className="w-3.5 h-3.5" />
                  <span>Prev Comp</span>
                </button>
                {currentInspectIndex >= 0 && (
                  <span className="px-2 py-1 rounded-lg bg-slate-100 dark:bg-slate-800/80 text-slate-700 dark:text-slate-300 text-xs font-mono font-bold border border-slate-200 dark:border-slate-700">
                    Slot {currentInspectIndex + 1} of {presentedMatches.length}
                  </span>
                )}
                <button
                  ...
                  disabled={!canNavigateNextComp}
                  title="Next comparable precedent (] or →)"
                  className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl border text-xs font-sans font-semibold transition-all cursor-pointer disabled:opacity-40 disabled:cursor-not-allowed border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800/90 text-slate-700 dark:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-700 active:scale-95 shadow-2xs"
                >
                  <span>Next Comp</span>
